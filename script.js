...
async function generatePdfBlob() {
  ...
  // ---- Part II ----
  heading("Part II \u00b7 Project Description");
  paragraph(state.projectDescription);

  // ---- Part III ----
  heading("Part III \u00b7 List of Violations");
  ...
  // ---- Part IV ----
  heading("Part IV \u00b7 Summary & Recommendations");

  const counts = {};
  HEURISTICS.forEach(h => counts[h.id] = [0, 0, 0, 0, 0]);
  state.issues.forEach(issue => {
    const sev = parseInt(issue.severity, 10);
    if (counts[issue.heuristic]) counts[issue.heuristic][sev]++;
  });

  const tableBody = HEURISTICS.map(h => {
    const row = counts[h.id];
