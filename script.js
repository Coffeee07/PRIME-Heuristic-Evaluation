...
function addIssueCard(data = {}) {
  ...
  function renderThumbs() {
    ...
    storedImages.forEach((src, i) => {
      ...
      img.addEventListener("click", () => { storedImages.splice(i, 1); renderThumbs(); saveDraft(); });
...
// renders it into a PDF Blob using html2pdf.js (only loaded when Firebase
// is enabled — see README.md).
...
// ---- Cloud save (optional Firebase hook — see README.md) ----
...
document.getElementById("cloudSaveBtn").addEventListener("click", async () => {
  ...
  if (typeof window.savePrimeEvaluationToCloud !== "function") {
    status.textContent = "☁️ Database isn't connected yet. See README.md to add Firebase, or use Export JSON / Save as PDF for now.";
    return;
  }
  ...
  try {
    ...
    console.error(e);
    status.textContent = "⚠️ Could not submit — check your Firebase setup in firebase-config.js.";
  }
});
