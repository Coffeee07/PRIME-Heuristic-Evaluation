...
// ---- "Where" options, dependent on the chosen Platform + User side ----
// Each entry is either a flat array of screen/step names, or an array of
// { group, items } objects to render as <optgroup>s.
const WHERE_OPTIONS = {
  lgu: {
    web: [
      { group: "Overview", items: ["Dashboard", "Analytics"] },
      { group: "Capacity Building", items: ["Assessments", "Responses", "Training Materials", "Calculator", "MERP Toolkit"] },
      { group: "Disaster Monitoring", items: ["Control Room", "Post-Disaster", "Untargetable MSME", "Settings"] },
      { group: "Content", items: ["News Updates"] }
    ],
    mobile: [
      { group: "Overview", items: ["Dashboard", "Analytics"] },
      { group: "Capacity Building", items: ["Assessments", "Responses"] },
      { group: "Disaster Monitoring", items: ["Hazard Map", "Control Room", "Post-Disaster"] }
    ]
  },
  microentrepreneur: {
    web: ["Profile", "Assessments", "Analytics Report", "Mapping", "Training Programs", "Financial Calculator", "News and Updates"],
    mobile: ["Home", "Disaster Info", "RiskMetrics", "Resources"]
  }
};

function flattenWhereItems(side, platform) {
  const options = (WHERE_OPTIONS[side] && WHERE_OPTIONS[side][platform]) || [];
  const flat = [];
...
// Rebuilds the Where <select> options for a card based on its current
// Platform + User side, then tries to preserve/restore `selectedValue`.
function populateWhereSelect(card, selectedValue) {
  const platform = card.querySelector(".issue-platform").value === "web" ? "web" : "mobile";
  const side = card.querySelector(".issue-side").value === "lgu" ? "lgu" : "microentrepreneur";
  const select = card.querySelector(".issue-where");
  const customInput = card.querySelector(".issue-where-custom");
  const options = (WHERE_OPTIONS[side] && WHERE_OPTIONS[side][platform]) || [];
  ...
  const flat = flattenWhereItems(side, platform);
  if (selectedValue && flat.includes(selectedValue)) {
...
function addIssueCard(data = {}) {
  ...
  populateWhereSelect(card, data.where || "");
  ...
  // Changing Platform or User side changes which "Where" options are valid,
  // so rebuild that dropdown (this resets the Where choice since the old
  // one may no longer apply to the new context).
  card.querySelector(".issue-platform").addEventListener("change", () => {
    populateWhereSelect(card, "");
    saveDraft();
  });
  card.querySelector(".issue-side").addEventListener("change", () => {
    populateWhereSelect(card, "");
    saveDraft();
  });

  // Show/hide the free-text field when "Other (please specify)" is picked.
  const whereSelect = card.querySelector(".issue-where");
  const whereCustom = card.querySelector(".issue-where-custom");
  whereSelect.addEventListener("change", () => {
    whereCustom.style.display = whereSelect.value === "other" ? "block" : "none";
    if (whereSelect.value !== "other") whereCustom.value = "";
    saveDraft();
  });
...
function collectState() {
  return {
    ...
    issues: Array.from(issuesList.querySelectorAll(".issue-card")).map(card => ({
      ...
      where: (() => {
        const sel = card.querySelector(".issue-where").value;
        return sel === "other" ? card.querySelector(".issue-where-custom").value : sel;
      })(),
